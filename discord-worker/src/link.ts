...
interface PanelLinkFailure {
  ok: false;
  errorCode: string;
  message: string;
}

type PanelLinkResult = PanelLinkSuccess | PanelLinkFailure;
...
function getIngestBaseUrl(): string {
  ...
  if (!url) {
    throw new Error(
      "INGEST_BASE_URL is required. Set it to https://losesperados.fr in production."
    );
  }
...
async function getMemberLinkData(
  discordId: string
): Promise<MemberLinkData | null> {
  const data = await panelFetch(`/api/staff/link/${discordId}`);
  if (!data || !("discordId" in data)) return null;
  return data as MemberLinkData;
}
...
async function updateMemberLink(
  ...
  if ("ok" in data && data.ok === false) {
    const errorValue = (data as PanelLinkError).error;
    const errorCode =
  ...
  } satisfies PanelLinkSuccess;
}

// Delete member link
async function deleteMemberLink(discordId: string): Promise<boolean> {
  const data = await panelFetch(`/api/staff/link/${discordId}`, {
    method: "DELETE",
  });
  return Boolean(data);
...
async function hasChefRole(interaction: ChatInputCommandInteraction | ButtonInteraction | ModalSubmitInteraction): Promise<boolean> {
...
function createLinkPanelEmbed(targetUser: string, linkData: MemberLinkData | null): EmbedBuilder {
  return buildLinkPanelEmbed({
    user: { id: targetUser, displayAvatarURL: () => "" } as User,
...
function createConfirmationEmbed(
  title: string,
  description: string,
  color: number
): EmbedBuilder {
...
function createLinkModal(discordId: string, currentData: MemberLinkData | null): ModalBuilder {
  ...
  modal.addComponents(
    ...
    new ActionRowBuilder<TextInputBuilder>().addComponents(rpNameInput)
  );
  ...
  if (!isChef) {
    await interaction.reply({
      embeds: [
        createErrorEmbed(
          "Accès Refusé",
          "Seuls les Chef Famille ou État-Major peuvent utiliser cette commande."
        ),
      ],
...
function parseLinkCustomId(customId: string): { action: string; targetId: string } | null {
...
export async function handleLinkButtonInteraction(
  ...
  // Handle delete button - show delete confirmation
    ...
    try {
      const confirmEmbed = createConfirmationEmbed(
        ...
        `Êtes-vous sûr de vouloir supprimer la liaison de <@${targetId}>?\n\nCette action est irréversible.`,
        0xff0000
      );
  ...
  // Handle confirm delete button - execute delete
    ...
    try {
      ...
      if (success) {
        ...
        await logToChannel(
          client,
          `🗑️ **Liaison Supprimée** - <@${interaction.user.id}> a supprimé la liaison de <@${targetId}>`
        );
      } else {
        await interaction.reply({
          embeds: [
            createErrorEmbed(
              "Erreur",
              "Impossible de supprimer la liaison (utilisateur non lié?)."
            ),
          ],
          flags: MessageFlags.Ephemeral,
        });
...
export async function handleLinkModalSubmission(
  ...
  try {
    // Get form values
    const steamIdRaw = interaction.fields.getTextInputValue(
      LINK_CUSTOM_IDS.STEAM_ID_INPUT
    );
    const rpNameRaw = interaction.fields.getTextInputValue(
      LINK_CUSTOM_IDS.RP_NAME_INPUT
    );
    ...
    if (!result) {
      await interaction.editReply({
        embeds: [
          createErrorEmbed(
            "Erreur API",
            "Impossible de créer/modifier la liaison. Vérifiez les données."
          ),
        ],
      });
    ...
    await logToChannel(
      client,
      `🔗 **Liaison Créée** - <@${interaction.user.id}> a lié <@${targetId}> (Steam: \`${steamId}\`, RP: **${rpName}**)`
    );
  } catch (e) {
...
export async function handleUnlinkCommand(
    ...
    await interaction.reply({
      embeds: [
        createErrorEmbed(
          ...
          "Vous ne pouvez pas retirer votre propre liaison."
        ),
      ],
      flags: MessageFlags.Ephemeral,
    });
  ...
  try {
    ...
    const confirmEmbed = createConfirmationEmbed(
      "Confirmer la suppression de la liaison",
      `Êtes-vous sûr de vouloir supprimer la liaison de <@${targetUser.id}>?\n\nCette action est irréversible.`,
      0xff0000
    );
...
export async function handleUnlinkButtonInteraction(
  interaction: ButtonInteraction,
  client: Client
): Promise<void> {
      ...
      if (success) {
        ...
        await interaction.followUp({
          embeds: [
            createSuccessEmbed(
              "Liaison Supprimée",
              `✅ La liaison de <@${targetId}> a été supprimée avec succès.`
            ),
          ],
          flags: MessageFlags.Ephemeral,
        });
        ...
        await logToChannel(
          client,
          `🗑️ **Liaison Supprimée** - <@${interaction.user.id}> a supprimé la liaison de <@${targetId}> via /unlink`
        );
      } else {
        // ✅ After deferUpdate, use followUp
        await interaction.followUp({
