 ...
 */
export function normalizeRpName(name: string | null | undefined): string {
  if (!name) return "";
 ...
 */
export function buildNickname(member: { rpName?: string | null }): string | null {
 ...
 */
export function shouldUpdateNickname(
  ...
  if (!desired && !currentNickname) return false;
