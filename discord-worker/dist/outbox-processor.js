...
export async function processOutboxQueue(prisma, discordClient) {
    try {
        ...
        for (const job of jobs) {
            try {
                await processJob(job, prisma, discordClient);
            }
            catch (error) {
...
async function processJob(job, prisma, discordClient) {
    ...
    switch (job.type) {
        case "RECRUITMENT_DECISION":
            await handleRecruitmentDecision(job, logsChannel);
            break;
        case "SANCTION_APPLY":
            await handleSanctionApply(job, discordClient);
            break;
        case "COMPLAINT_DECISION":
            await handleComplaintDecision(job, logsChannel);
            break;
        case "SEND_MESSAGE":
            await handleGenericMessage(job, logsChannel);
            break;
        case "SANCTION_NOTIFY":
            await handleSanctionNotify(job, logsChannel);
            break;
        default:
...
async function handleRecruitmentDecision(job, channel) {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: "Candidat", value: candidateRpName || "Unknown", inline: true }, { name: "Décision", value: decision === "ACCEPT" ? "✅ ACCEPTÉ" : "❌ REFUSÉ", inline: true }, { name: "Score", value: `${totalOn20 ?? "-"}/20 (${totalPoints ?? "-"} pts)`, inline: true })
        .setTimestamp();
    await channel.send({ embeds: [embed] });
}
async function handleComplaintDecision(job, channel) {
    ...
    const decisionInfo = labels[decision] || {
        label: decision,
        color: 0x6b7280,
    };
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: "Titre", value: complaintTitle || "Sans titre", inline: false }, { name: "Décision", value: decisionInfo.label, inline: true })
        .setTimestamp();
    await channel.send({ embeds: [embed] });
}
async function handleSanctionApply(job, discordClient) {
...
async function handleSanctionNotify(job, channel) {
    const { action, type, memberName, reason } = job.meta;
    const embed = new EmbedBuilder()
        .setTitle(`⚖️ Sanction ${action}`)
        .setColor(0xf59e0b)
        .addFields({ name: "Membre", value: memberName || "Unknown", inline: true }, { name: "Type", value: type, inline: true }, { name: "Raison", value: reason || "-", inline: false })
        .setTimestamp();
    await channel.send({ embeds: [embed] });
}
async function handleGenericMessage(job, channel) {
