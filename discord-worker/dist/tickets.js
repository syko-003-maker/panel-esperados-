...
import { CUSTOM_ID, IDS, EVENT_VERSION } from "./ids.js";
...
function getPanelUrl(type, ticketKey) {
...
// ─────────────────────────────────────────────────────────────
async function createTicketThread(parent, name, authorId) {
...
// Anti-spam check
// ─────────────────────────────────────────────────────────────
async function checkOpenLimit(type, discordId) {
...
function panelLinkRow(type, ticketKey) {
    return new ActionRowBuilder().addComponents(new ButtonBuilder()
...
// Modal openers
// ─────────────────────────────────────────────────────────────
export async function openRecruitmentModal(interaction) {
    ...
    const dispo = new TextInputBuilder()
        ...
        .setMinLength(5)
        .setMaxLength(500);
    modal.addComponents(new ActionRowBuilder().addComponents(steamId), new ActionRowBuilder().addComponents(rpName), new ActionRowBuilder().addComponents(motivation), new ActionRowBuilder().addComponents(dispo));
    await interaction.showModal(modal);
}
export async function openComplaintModal(interaction) {
    const modal = new ModalBuilder()
    ...
    const details = new TextInputBuilder()
        ...
        .setMinLength(10)
        .setMaxLength(1500);
    modal.addComponents(new ActionRowBuilder().addComponents(target), new ActionRowBuilder().addComponents(reason), new ActionRowBuilder().addComponents(details));
    await interaction.showModal(modal);
}
// ─────────────────────────────────────────────────────────────
// Modal submit handlers
// ─────────────────────────────────────────────────────────────
export async function handleRecruitmentSubmit(interaction) {
    ...
    const parent = await interaction.client.channels.fetch(IDS.TICKETS_PARENT_CHANNEL_ID);
    if (!parent || parent.type !== ChannelType.GuildText) {
        return interaction.editReply("❌ Parent tickets introuvable ou pas un salon texte.");
    }
    let ticketKey = makeTicketKey("R");
    const thread = await createTicketThread(parent, `recrutement-${ticketKey}`, interaction.user.id);
    ...
    const event = {
        ...
        payload: {
            ...
            discordDisplayName: interaction.member?.nickname || interaction.user.globalName || interaction.user.username,
        },
    };
    ...
    const contentParts = [];
    if (staffPing)
        contentParts.push(`${staffPing} Nouveau recrutement !`);
    if (!ing.ok)
        contentParts.push(`⚠️ Ingest KO: ${ing.error}`);
    const embed = new EmbedBuilder()
        ...
        .setColor(0x3b82f6)
        .addFields({ name: "👤 RP Name", value: rpName || "-", inline: true }, { name: "🎮 Steam ID", value: steamId || "-", inline: true }, { name: "🔢 Âge", value: "-", inline: true }, { name: "⚠️ Sanctions actives", value: "0", inline: true }, { name: "🆔 Discord", value: `<@${interaction.user.id}>`, inline: true }, { name: "Ticket", value: ticketKey, inline: true }, { name: "Motivation", value: motivation.slice(0, 1024), inline: false }, { name: "Dispos", value: dispo.slice(0, 1024), inline: false })
...
export async function handleComplaintSubmit(interaction) {
    await interaction.deferReply({ ephemeral: true });
    // Rate limit check (cooldown)
    ...
    const thread = await createTicketThread(parent, `plainte-${ticketKey}`, interaction.user.id);
    // If thread creation fails entirely, inform user
    if (!thread) {
    ...
    const contentParts = [];
    if (staffPing)
        contentParts.push(`${staffPing} Nouvelle plainte !`);
    if (!ing.ok)
        contentParts.push(`⚠️ Ingest KO: ${ing.error}`);
    const embed = new EmbedBuilder()
        .setTitle("📌 Plainte")
        .setColor(0xef4444)
        .addFields({ name: "Ticket", value: ticketKey, inline: true }, { name: "Auteur", value: `${interaction.user.tag} (<@${interaction.user.id}>)`, inline: false }, { name: "Cible", value: target || "—", inline: false }, { name: "Raison", value: reason, inline: false }, { name: "Détails", value: details.slice(0, 1024), inline: false })
        .setFooter({ text: `Thread: discussion uniquement ici • ${ticketKey}` })
        .setTimestamp();
    await thread.send({
        content: contentParts.length > 0 ? contentParts.join("\n") : undefined,
...
async function postCloseLog(opts) {
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(color)
            .addFields({ name: "Type", value: opts.type, inline: true }, { name: "Ticket", value: opts.ticketKey, inline: true }, { name: "Status", value: opts.statusText, inline: true }, { name: "Staff", value: opts.staffText, inline: false }, { name: "Thread", value: `<#${opts.threadId}>`, inline: true }, { name: "Panel", value: getPanelUrl(opts.type, opts.ticketKey), inline: true })
            .setTimestamp();
        if (opts.authorText) {
...
// ─────────────────────────────────────────────────────────────
export async function handleStaffButtons(interaction) {
    // Recruitment finish
    if (interaction.customId.startsWith(CUSTOM_ID.STAFF_RECRUIT_FINISH_PREFIX)) {
        await interaction.deferReply({ ephemeral: true });
        const ticketKey = interaction.customId.replace(CUSTOM_ID.STAFF_RECRUIT_FINISH_PREFIX, "");
        ...
        return interaction.editReply(ing.ok
            ? "✅ Recrutement clôturé."
    ...
    if (interaction.customId.startsWith(CUSTOM_ID.STAFF_COMPLAINT_CLOSE_PREFIX)) {
        ...
        const rest = interaction.customId.replace(CUSTOM_ID.STAFF_COMPLAINT_CLOSE_PREFIX, "");
        const colonIdx = rest.indexOf(":");
        const status = rest.slice(0, colonIdx);
