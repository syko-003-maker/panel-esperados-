...
const ALLOWED_ROLES = [IDS.CHEF_FAMILLE_ROLE_ID, IDS.ETAT_MAJOR_ROLE_ID].filter((id) => typeof id === "string" && ROLE_ID_REGEX.test(id));
 ...
 */
function checkPermissions(member, clickerId, requesterDiscordId) {
 ...
 */
export async function handleLinkRequestAction(client, options) {
 ...
 */
async function updateLinkRequestMessage(message, action, clickerId, clickerName) {
 ...
 */
async function updateLinkRequestMessageAlreadyHandled(message, status) {
    if (!message || !message.embeds || !message.embeds[0]) {
        console.warn("[linkreq] Message not found or no embeds to update");
        return;
    }
 ...
 */
export async function sendLinkRequestDecisionMessage(message, action, clickerId, requesterDiscordId) {
    const actionConfig = {
        accept: "✅ Acceptée",
        refuse: "❌ Refusée",
        archive: "📦 Archivée",
 ...
 */
export async function sendLinkAcceptedDM(client, requesterDiscordId) {
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            value: "Rendez-vous sur https://losesperados.xyz/me pour voir votre nouveau statut",
        })
            .setTimestamp();
        await user.send({
            embeds: [embed],
