...
const renameRateLimitMap = new Map();
 ...
 */
function checkRenameRateLimit(discordId) {
    const now = Date.now();
 ...
 */
export function initWorkerServer(client) {
    app = express();
     ...
     * }
     */
    app.post("/api/worker/contact-notification", async (req, res) => {
        try {
     ...
     * }
     */
    app.post("/api/worker/link-request/post", async (req, res) => {
        try {
            // Validate secret
            const xSecret = req.headers["x-ingest-secret"];
            const isValidXSecret = WORKER_SECRET && xSecret === WORKER_SECRET;
    ...
    app.post("/internal/discord/postMessage", async (req, res) => {
        try {
            ...
            if (!WORKER_SECRET) {
                ...
                logError("post_message_auth_error", `Worker secret not configured (INGEST_SECRET: ${hasIngestSecret}, DISCORD_WORKER_SECRET: ${hasWorkerSecret})`);
     ...
     * }
     */
    app.post("/internal/discord/rename", async (req, res) => {
        try {
            const xSecret = req.headers["x-ingest-secret"];
            const isValidXSecret = WORKER_SECRET && xSecret === WORKER_SECRET;
            if (!WORKER_SECRET) {
                ...
                const hasIngestSecret = !!process.env.INGEST_SECRET;
                const hasWorkerSecret = !!process.env.DISCORD_WORKER_SECRET;
                logError("rename_auth_error", `Worker secret not configured (INGEST_SECRET: ${hasIngestSecret}, DISCORD_WORKER_SECRET: ${hasWorkerSecret})`);
                return res.status(401).json({ ok: false, error: "Server not configured" });
            }
            if (!isValidXSecret) {
            ...
            // Attempt rename
            const result = await renameMemberIfPossible(client, guildId, discordId, rpName, reason);
            log("rename_result", {
                discordId,
                ok: result.ok,
