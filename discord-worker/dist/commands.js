 ...
 */
import { REST, Routes, SlashCommandBuilder, PermissionFlagsBits, EmbedBuilder, } from "discord.js";
import { PrismaClient } from "@prisma/client";
import { IDS } from "./ids.js";
import { runManualSync } from "./syncRoles.js";
import { postLinkPanel } from "./contactPanel.js";
import { createLinkCommand, handleLinkCommand, createUnlinkCommand, handleUnlinkCommand, } from "./link.js";
// ─────────────────────────────────────────────────────────────
// Prisma Setup
// ─────────────────────────────────────────────────────────────
...
const commands = [
    ...
    new SlashCommandBuilder()
        ...
        .addUserOption((opt) => opt.setName("user").setDescription("Le membre Discord à rechercher").setRequired(true))
    ...
    new SlashCommandBuilder()
        ...
        .addStringOption((opt) => opt.setName("ticketkey").setDescription("La clé du ticket (ex: REC-001)").setRequired(true))
        .setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages)
        .setDMPermission(false),
    // /sanction - Staff only
    new SlashCommandBuilder()
        ...
        .addUserOption((opt) => opt.setName("user").setDescription("Le membre à sanctionner").setRequired(true))
        .addStringOption((opt) => opt
        .setName("type")
        .setDescription("Type de sanction")
        .setRequired(true)
        .addChoices({ name: "Avertissement", value: "WARNING" }, { name: "Sanction légère", value: "LIGHT" }, { name: "Sanction lourde", value: "HEAVY" }, { name: "Suspension temporaire", value: "TEMP_BAN" }))
        .addStringOption((opt) => opt.setName("reason").setDescription("Raison de la sanction").setRequired(true))
...
function getStaffRoleIds() {
    const roleIds = IDS.STAFF_ROLE_IDS;
    return roleIds.filter((id) => typeof id === "string" && ROLE_ID_REGEX.test(id));
}
async function isStaff(interaction) {
    const staffRoleIds = getStaffRoleIds();
    if (staffRoleIds.length === 0)
        return true; // No staff role configured = allow
...
// /syncroles Handler
// ─────────────────────────────────────────────────────────────
async function handleSyncRolesCommand(interaction, client) {
    ...
    try {
        ...
        if (result.changes.length > 0 && result.changes.length <= 10) {
            const changesText = result.changes
                ...
                const parts = [];
                if (c.added.length > 0)
                    parts.push(`+${c.added.length} rôle(s)`);
                if (c.removed.length > 0)
                    parts.push(`-${c.removed.length} rôle(s)`);
...
// /linkpanel Handler
// ─────────────────────────────────────────────────────────────
async function handleLinkPanelCommand(interaction, client) {
    if (!(await requireStaff(interaction)))
        return;
    await interaction.deferReply({ ephemeral: true });
...
async function handleMemberCommand(interaction) {
    ...
    try {
        ...
        const statusEmoji = activity?.status === "OK" ? "🟢" : activity?.status === "WARN" ? "🟡" : activity?.status === "KO" ? "🔴" : "⚪";
        const embed = new EmbedBuilder()
        ...
        const links = [];
        if (m.links?.member)
            links.push(`[📋 Fiche membre](${m.links.member})`);
        if (m.links?.sanctions)
            links.push(`[⚠️ Sanctions](${m.links.sanctions})`);
        if (m.links?.activity)
            links.push(`[📊 Activité](${m.links.activity})`);
        if (links.length > 0) {
...
async function handleTicketCommand(interaction) {
    ...
    try {
        ...
        const statusColor = t.status === "PENDING" || t.status === "OPEN"
            ? 0xffa500
        ...
        const links = [];
        if (t.links?.panel)
            links.push(`[📋 Panel](${t.links.panel})`);
        if (t.links?.thread)
            links.push(`[💬 Thread](${t.links.thread})`);
        if (links.length > 0) {
            embed.addFields({ name: "Liens", value: links.join(" • "), inline: false });
        }
        await interaction.editReply({ embeds: [embed] });
...
// /sanction Handler
// ─────────────────────────────────────────────────────────────
async function handleSanctionCommand(interaction) {
    if (!(await requireStaff(interaction)))
        return;
    await interaction.deferReply({ ephemeral: true });
    const targetUser = interaction.options.getUser("user", true);
...
async function handleActivityCommand(interaction) {
    ...
    try {
        ...
        const statusEmoji = snapshot?.status === "OK" ? "🟢" : snapshot?.status === "WARN" ? "🟡" : snapshot?.status === "KO" ? "🔴" : "⚪";
        const embed = new EmbedBuilder()
...
// ─────────────────────────────────────────────────────────────
async function handleSyncNameCommand(interaction, client) {
    if (!(await requireStaff(interaction)))
        return;
    await interaction.deferReply({ ephemeral: true });
    try {
        if (!WORKER_SECRET) {
            await interaction.editReply({
                content: "❌ INGEST_SECRET manquant côté worker. Contactez un administrateur.",
            });
            return;
        }
        ...
        const memberResponse = await fetch(`${PANEL_BASE_URL}/api/ingest/members/by-discord/${discordId}`, {
            headers: {
                "x-ingest-secret": WORKER_SECRET,
                "Content-Type": "application/json",
            },
        ...
        if (!memberResponse.ok) {
            ...
            throw new Error(`Fetch failed: ${memberResponse.status} ${memberResponse.statusText}`);
        }
 ...
 */
async function computeGlobalBalance(steamId) {
 ...
 */
async function isChefOrStaff(userId, guild) {
    if (!guild)
        return false;
    try {
        ...
        const staffRoleIds = IDS.STAFF_ROLE_IDS || [];
...
async function handleBankCommand(interaction) {
    try {
        ...
        const fmtMoney = (n) => Math.trunc(n).toLocaleString("fr-FR");
        const formatDate = (d) => d.toLocaleDateString("fr-BE", {
            day: "2-digit",
            month: "2-digit",
            year: "numeric",
            hour: "2-digit",
