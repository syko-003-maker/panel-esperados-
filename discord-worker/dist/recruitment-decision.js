...
function parseRecruitmentCustomId(customId) {
...
async function callDecisionAPI(ticketKey, decision, staffDiscordId, staffUsername, messageId, channelId) {
    try {
        ...
        logError("recruitment_api_call_failed", { ticketKey, decision }, error);
...
function buildDecisionEmbed(params) {
    ...
    const embed = new EmbedBuilder()
        .setTitle(title)
        .setColor(color)
        .addFields({ name: "👤 Candidat", value: candidateName, inline: true }, { name: "🎫 Ticket", value: `\`${ticketKey}\``, inline: true }, { name: "📋 Décision", value: isApproved ? "**ACCEPTÉ**" : "**REFUSÉ**", inline: true })
        .setTimestamp();
    if (steamId) {
...
async function postDecisionLog(params) {
    ...
    try {
        ...
        logError("recruitment_log_post_failed", { ticketKey: params.ticketKey }, error);
    }
}
/**
 * Main handler for recruitment decision buttons
 */
export async function handleRecruitmentDecision(interaction) {
    ...
    try {
        ...
        logError("recruitment_ack_failed", { customId: interaction.customId }, ackError);
        return;
    }
    // Parse customId
    ...
    try {
        ...
        logError("recruitment_decide_error", { ticketKey, decision }, error);
        try {
