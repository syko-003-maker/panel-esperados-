        ...
        {staff.length === 0 ? (
          ...
          staff.map((s, i) => (
            ...
            >
              ...
              <span className={`${rankColor(s.rankStaff)}`}>{s.rankStaff}</span>
              <span className="text-slate-500">· {formatJob(s.job)}</span>
              {s.server != null && (
                <span className="rounded border border-white/10 bg-white/5 px-1 text-[10px] text-slate-400">S{s.server}</span>
              )}
            </span>
          ))
        )}
