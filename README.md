...
| | |
|---|---|
| **~113 000** lignes de TypeScript | **2** services en production (web + worker) |
| **247** routes API | **60** modèles de données (Prisma) |
| **80** pages | **23** migrations versionnées |
...
| **286** commits — **1** développeur | **0 €** d'API externes (IA incluse) |
...
- **Nom RP forcé** : le sync réécrit le nom depuis le jeu toutes les 45 s — un nom saisi dans le panel tient, et se relâche en un clic
- **Masquage d'un membre** : retiré de tous les affichages (listes, réunions, sanctions, hiérarchie Discord) sans rien perdre de son grade ni de ses droits
...
- **4 fiches de spécialité** publiques (construction, négociation, conduite, leader de scène)
...
- **Logs tickets au rendu unifié** (couleur = sens, avatar en en-tête)
...
- **📵 Trois pièges du mode application, trouvés à la dure.** Un service worker qui met en cache le HTML de Next.js finit par référencer des **chunks JS supprimés** au déploiement suivant : la page s'affiche, React n'hydrate jamais, plus **aucun clic ne répond** — et l'appli installée, qui garde son cache entre les lancements, reste bloquée là alors que le site va bien. Second piège : `perspective` (ou tout `transform` / `filter`) sur un **conteneur de layout** en fait le bloc de référence de ses descendants en `position: fixed` — toutes les fenêtres modales se retrouvaient décalées, et rognées par l'`overflow-hidden` du shell. La profondeur 3D est désormais portée par la fonction `perspective()` **dans le `transform` de l'élément**, jamais sur un parent. Troisième piège : un `useEffect(..., [])` ne se rejoue **qu'au montage**. Sur le site on navigue, le composant se remonte, les données sont fraîches — mais l'application installée n'est **jamais rechargée**, seulement mise en veille puis reprise. L'écran restait donc figé sur les valeurs du jour où l'appli avait été ouverte, d'où « le playtime bouge sur le site mais pas sur l'appli » : même API, seul le moment de l'appel différait. Un `visibilitychange` avec un intervalle minimum recharge au retour au premier plan, sans écran de chargement.
- **🖼️ Avatars increvables — après avoir fusionné quatre systèmes en un.** Les photos manquaient de façon récurrente. La cause n'était pas la donnée : **quatre** systèmes interrogeaient Discord chacun de leur côté, avec leur propre cache. Une liste de 40 membres produisait des dizaines d'appels en moins d'une seconde ⇒ **HTTP 429 garanti** — et l'échec était mis en cache **une heure**. Tout passe désormais par un moteur unique : file d'attente **sérialisée**, fenêtre de repos partagée après un 429, et une règle qui ne se contourne plus — un **échec réseau n'est jamais mis en cache** (`undefined` = « on ne sait pas », seul un 404 vaut « pas d'avatar »). Détail trouvé à la dure : le préfixe `a_` d'un avatar animé **survit à l'expiration du Nitro** alors que l'asset, lui, disparaît — d'où un `.gif` en **415** et une image vide. On sert donc toujours `.png`.
...
- **📉 Un invariant métier vaut mieux qu'une heuristique de panne.** Le playtime hebdomadaire est lu chez le tiers ; un garde-fou refusait d'écrire sur relevé **vide**, mais un relevé **partiel** (1 entrée sur 231) le franchissait et remettait les 230 autres à zéro — le temps de jeu de la semaine effacé, potentiellement la veille de la réunion qui s'en sert pour compter les présences. Plutôt que de tenter de détecter la panne, on s'appuie sur une propriété du compteur : **cumulé depuis lundi 00:00, il ne peut que monter sur une même semaine**. Toute baisse intra-semaine est donc un artefact, jamais une donnée — la valeur déjà enregistrée cette semaine fait **plancher**. L'élégance est là : aucune distinction « panne » / « nouvelle semaine » n'est nécessaire, le plancher tombe **tout seul** au passage du lundi puisque la valeur stockée date d'avant. Deux compteurs dans les logs signalent quand la protection joue — les données restent saines, mais cessent de progresser tant que le tiers répond mal.
- **🎭 Masquer quelqu'un sans rien lui retirer.** Sortir une personne de **tous** les affichages (listes, réunions, sanctions, statistiques, hiérarchie Discord) tout en lui laissant grade, rôles et droits. Le point d'accroche est le prédicat racine du périmètre membre, ce qui couvre l'essentiel en une ligne — mais pas les portées « démoté / blacklisté / réserviste », qui ne passent pas par cette racine et l'auraient fait réapparaître. Piège SQL au passage : `NOT IN` sur une colonne nullable écarte **aussi** les lignes `NULL`, ce qui faisait disparaître les membres sans compte Discord. Trouvé en comptant avant/après plutôt qu'en constatant que ça compilait.
