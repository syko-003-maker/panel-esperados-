...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  void sendPushToStaff({
    title: "📝 Justification de sanction",
    body: `${scope.rpName || "Un membre"} a justifié sa sanction${sanction.type ? ` (${sanction.type})` : ""}.`,
    url: "/staff/sanctions",
    tag: "justif-sanction-" + justification.id,
  }).catch((err: unknown) => {
    // Notification perdue : degradation silencieuse d'un canal d'alerte.
    console.warn("[push] notification non delivree", {
      event: "sanction_justified",
