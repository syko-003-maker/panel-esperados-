  ...
  try {
    ...
    if (!discordIdsParam) {
      return NextResponse.json(
        { error: "Missing ?discordIds parameter" },
        { status: 400 }
      );
    }

    const discordIds = discordIdsParam
      .split(",")
      .map((id) => id.trim())
      .filter(Boolean);
    ...
    if (discordIds.length === 0) {
      return NextResponse.json(
        { error: "No valid Discord IDs provided" },
        { status: 400 }
      );
    }

    debug("[member-status] Checking status for members", { count: discordIds.length });

    const result: MemberStatusResult = {};

    // Process in concurrent batches (5 at a time to avoid rate limits)
    const CONCURRENCY = 5;
    ...
    const activeSummary = Object.values(result).filter((s) => s === "active")
      .length;
    const formerSummary = Object.values(result).filter((s) => s === "former")
      .length;
    const notFoundSummary = Object.values(result).filter((s) => s === "not-found")
      .length;
    const unavailableSummary = Object.values(result).filter((s) => s === "unavailable")
      .length;

    debug("[member-status] Summary:", {
      total: discordIds.length,
      active: activeSummary,
      former: formerSummary,
    ...
    debug("[member-status] Unhandled error:", {
      error: error instanceof Error ? error.message : String(error),
    });
    return NextResponse.json(
      {
