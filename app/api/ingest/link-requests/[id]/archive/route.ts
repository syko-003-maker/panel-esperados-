...
export async function POST(
  ...
  try {
    ...
    console.log("[link-request:archive]", {
      id,
      clickerId,
      clickerName,
      reason,
    ...
    if (linkRequest.status !== "PENDING" && linkRequest.status !== "OPENED") {
      console.log("[link-request:archive] Already handled", {
        id,
    ...
    // Update LinkRequest to ARCHIVED
    ...
    console.log("[link-request:archive] Success", {
      id,
    ...
    console.error("[link-request:archive] Error:", error);
    return NextResponse.json(
      {
        ok: false,
